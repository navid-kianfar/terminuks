...
const TerminalView = ({ sessionId }: TerminalViewProps) => {
  ...
  const setupMockTerminal = useCallback((terminal: XTerm) => {
    terminal.clear();
    terminal.writeln(
      isLocalSession
    ...
    if (isLocalSession) {
      terminal.writeln('\x1b[0;37mThe local shell backend is unavailable right now, so this tab has fallen back to preview mode.\x1b[0m');
    }
    terminal.write('\r\n$ ');
  ...
  const connectLocalShell = useCallback(async (terminal: XTerm) => {
  ...
  const connectSSH = useCallback(async (terminal: XTerm, nextHost: NonNullable<typeof host>) => {
    cleanupListenersRef.current.forEach((cleanup) => cleanup());
    cleanupListenersRef.current = [];
    inputDisposableRef.current?.dispose();
    inputDisposableRef.current = null;
    ...
    updateSession(sessionId, { status: 'connecting', lastError: undefined, streamId: undefined });
    setSessionStatus('connecting');
    setStatusMessage(`Connecting to ${nextHost.username}@${nextHost.address}:${nextHost.port}...`);
    setTrustPrompt(null);
    setShowConnectionLogs(false);
    setConnectionLogs([
    ...
    try {
      if (!window.electron) {
        ...
        terminal.writeln(`\x1b[1;33mPlease use the Electron app for full SSH functionality.\x1b[0m`);
        setupMockTerminal(terminal);
        return;
      }
            ...
            if (error instanceof Error && error.message.includes('Stream not found')) {
              try {
                ...
                const recoveryMessage =
                  recoveryError instanceof Error ? recoveryError.message : 'Failed to recover SSH stream';
  ...
  useEffect(() => {
    ...
    if (isLocalSession) {
      ...
      terminal.writeln('\x1b[0;37mCreate terminals from the picker to bind them to a saved host.\x1b[0m');
    }

    const handleResize = () => {
      fitAddon.fit();
      if (isLocalSession && localStreamIdRef.current && window.electron && xtermRef.current) {
        window.electron.localShell.resize(
          ...
          <div className="terminal-connection-overlay">
            <div className="terminal-connection-card">
              ...
              <div className="terminal-connection-progress">
                ...
                <div className="terminal-connection-stage">
                  <span className={`step ${effectiveTrustPrompt ? 'warning active' : connectionStage > 0 ? 'active' : connectionStage < 0 ? 'error' : ''}`} />
                  <span>Verify Identity</span>
                </div>
                <span className={`line ${connectionStage > 1 ? 'active' : connectionStage < 0 ? 'error' : ''}`} />
                <div className="terminal-connection-stage">
                  <span className={`step ${effectiveSessionStatus === 'reconnecting' ? 'active' : effectiveSessionStatus === 'error' ? 'error' : effectiveSessionStatus === 'connecting' || effectiveTrustPrompt ? '' : 'complete'}`} />
              ...
              <div className="terminal-connection-actions">
                <Button variant="outline" size="sm" onClick={() => setShowConnectionLogs((value) => !value)}>
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          appendConnectionLog(`Trusted host fingerprint ${effectiveTrustPrompt.fingerprint}`);
                          setTrustPrompt(null);
                          if (xtermRef.current) {
                            connectSSH(xtermRef.current, host);
                          }
              ...
              {showConnectionLogs && connectionLogs.length > 0 && (
                <pre className="terminal-connection-logs">
                  {connectionLogs.join('\n')}
                </pre>
              )}
            </div>
